...
/**
 * The RESTRequest class simplifies the process of making RESTful requests to
...
public class RESTRequest extends AsyncTask<Void, Void, String> {
	...
	public enum HeaderAcceptedData {
		TEXT("text/plain"), HTML("text/html"), JSON("application/json"), XML(
				"application/xml");
	...
	public enum ExceptionCode {
		UNKNOWN_METHOD(1), INVALID_URL(2), INVALID_PARAMETERS(3), REQUEST_FAILED(
				4), REQUEST_ABORTED(5), NO_RESULT(6);
	...
	/**
	 * This variable indicates whether or not the httpRequest was manually
	 * aborted using the abort() method.
	 */
	...
	/**
	 * The ID to pass along with the event. This helps recognizing an event
	 * fired by a class that needs to handle multiple REST requests.
	 */
	protected String ID;
	...
	/**
	 * Overloads the RESTRequest(String address, Method method, String ID)
	 * constructor.
	 * 
	 * @param address
	 *            ,
	...
	/**
	 * Overloads the RESTRequest(String address, Method method, String
	 * headerAcceptedData, String ID) constructor.
	 * 
	 * @param address
	 * @param method
	 * @param ID
	...
	public RESTRequest(String address, Method method, String ID) {
		this(address, method, HeaderAcceptedData.JSON.getHeaderAcceptedData(),
				ID);
	}

	/**
	 * Constructs a new RESTRequest with the address, method, and accepted data
	 * type.
	 * 
	 * The passed ID will be sent along with the fired event to be able to
	 * recognize the specific event in a class that handles multiple REST
	 * requests.
	 * 
	 * @param address
	 * @param method
	 ...
	 * @param ID
	 */
	public RESTRequest(String address, Method method,
			String headerAcceptedData, String ID) {
	...
	protected String doInBackground(Void... voids) {
		...
		try {
			...
			// Trick to read all data from a stream in one line:
	...
	protected void onPreExecute() {
		...
		for (RESTRequestListener eventListener : eventListeners) {
			...
			eventListener
					.RESTRequestOnPreExecute(new RESTRequestEvent(this, ID));
		}
	}
	...
	protected void onProgressUpdate(Void... voids) {
		...
		for (RESTRequestListener eventListener : eventListeners) {
			// Create new RESTRequestEvent to be handled by the event listener
			eventListener.RESTRequestOnProgressUpdate(new RESTRequestEvent(
					this, ID));
		}
	}
	...
	protected void onPostExecute(String result) {
		...
		for (RESTRequestListener eventListener : eventListeners) {
			// Create new RESTRequestEvent to be handled by the event listener
			eventListener.RESTRequestOnPostExecute(new RESTRequestEvent(this,
					result, ID));
		}
	}
	...
	private String encode(String value) {
		String returnSting = "";
		try {
			returnSting = Base64.encodeToString(value.getBytes("UTF-8"),
					Base64.NO_WRAP);
		} catch (Exception e) {
			e.printStackTrace();
		}
